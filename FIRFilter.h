...
namespace NTSC
{
  ...
  public:
    ...
    static FIRFilter Convolve(const FIRFilter &a, const FIRFilter &b)
    {
      ...
      f.coefficients.resize(Math::AlignInt(2*k_initialZeroPadding + f.actualFilterLength, k_maxFloatAlignment));
    ...
    static FIRFilter CreateLowPassFromRolloffCenterAndLength(f32 rolloffCenter, u32 filterLength, bool useWindow = true)
    {
      ...
      f.coefficients.resize(Math::AlignInt(filterLength + 2*k_initialZeroPadding, k_maxFloatAlignment));
      memset(f.coefficients.data(), 0, f.coefficients.size() * sizeof(f.coefficients[0]));
      ...
      {
        f32 result = Math::SincPi(rolloffCenter * (2.0f * f32(i) - f32(filterLength - 1)));

        if (useWindow)
        {
          result *= 0.42f - 0.5f * Math::CosPi(2.0f * f32(i) / f32(filterLength - 1)) + 0.08f * Math::CosPi(4.0f * f32(i) / f32(filterLength - 1));
        }
  ...
  template <bool UseAVX2>
    void ProcessAvx(const AlignedVector<f32> &valuesIn, size_t count, AlignedVector<f32> *pValuesOut) const
    {
      ...
      {
        u32 halfLength = (Length() - 1) / 2;
        u32 firstFullSample = Math::AlignInt(halfLength, k_setSize);
        firstFullSet = firstFullSample / k_setSize;
        u32 filterOffset = (firstFullSample - halfLength) % k_setSize;
        filterSetCount = Math::AlignInt(actualFilterLength + filterOffset, k_setSize) / k_setSize;
