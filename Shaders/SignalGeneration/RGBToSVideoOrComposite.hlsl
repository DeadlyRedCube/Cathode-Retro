...
float4 Main(float2 signalTexCoord)
{
  ...
  float3 rgb = SAMPLE_TEXTURE(g_sourceTexture, g_sampler, texCoord).rgb;
  ...
  float2 scanlinePhase = SAMPLE_TEXTURE(g_scanlinePhases, g_sampler, (float2(signalTexelIndex.y + 0.5, 0) / g_scanlineCount)).xy;
