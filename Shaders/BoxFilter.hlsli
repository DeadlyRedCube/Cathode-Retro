  ...
  out float4 centerSample)
{
  ...
  centerSample = SAMPLE_TEXTURE(sourceTexture, samp, texCoord);

  // Average starts with the center sample
  float4 avg = centerSample;
  ...
  if (remainder == 3U)
  {
    ...
    avg += 1.5 * SAMPLE_TEXTURE(sourceTexture, samp, (texCoord + float2(iterEnd + 1.0 / 3.0, 0) * invTextureSize));
    avg += 1.5 * SAMPLE_TEXTURE(sourceTexture, samp, (texCoord - float2(iterEnd + 1.0 / 3.0, 0) * invTextureSize));
  }
  else if (remainder > 0U)
  {
    ...
    avg += scale * SAMPLE_TEXTURE(sourceTexture, samp, (texCoord + float2(iterEnd + 1.0, 0) * invTextureSize));
    avg += scale * SAMPLE_TEXTURE(sourceTexture, samp, (texCoord - float2(iterEnd + 1.0, 0) * invTextureSize));
  }
