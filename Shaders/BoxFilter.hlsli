  ...
  out float2 centerSample)
{
  ...
  centerSample = sourceTexture.SampleLevel(samp, texCoord, 0);
  ...
  {
    avg += 2 * sourceTexture.SampleLevel(samp, texCoord + float2(i - 0.5, 0) * invTextureSize, 0);
    avg += 2 * sourceTexture.SampleLevel(samp, texCoord + float2(0.5 - i, 0) * invTextureSize, 0);
  }
  ...
  if (remainder == 3)
  {
    ...
    avg += 1.5 * sourceTexture.SampleLevel(samp, texCoord + float2(iterEnd + 1.0 / 3.0, 0) * invTextureSize, 0);
    avg += 1.5 * sourceTexture.SampleLevel(samp, texCoord + float2(-iterEnd - 1.0 / 3.0, 0) * invTextureSize, 0);
  }
  else if (remainder > 0)
  {
    ...
    avg += scale * sourceTexture.SampleLevel(samp, texCoord + float2(iterEnd + 1.0, 0) * invTextureSize, 0);
    avg += scale * sourceTexture.SampleLevel(samp, texCoord + float2(-iterEnd - 1.0, 0) * invTextureSize, 0);
  }
