...
{
  // A simple approximation of atan2 will suffice here, just a few terms of the taylor series are good enough for the angle ranges we're
  //  dealing with.
  x /= y;
  float2 x2 = x*x;
  ...
  float2 distortion)
{
  ...
  // We're going to cast a ray from a virtual camera position p0 (0, 0, -k_distance) and collide it with a unit sphere. The horizontal and
  //  vertical spread of the rays is determined by the distortion values.
  ...
  //  p = p0 + r*t
  // Since we have a non-translated unit sphere (1 = x^2 + y^2 + z^2) we can sub in our parametric values and solve for t as a quadratic.
  //  To simplify the math, we're going to use a slightly different form of quadratic equation: t^2 - 2b*t + c == 0:
  ...
  // Get the nearer of the two coordinates (and do a max inside the sqrt so we still get continuous values outside of the sphere, even if
  //  they're nonsensical, they'll get masked out by our mask value anyway).
  ...
  // Do the same calculation as above, but for two additional rays: the x, 0, z ray pointing all the way to the right, and the 0, y, z
  //  ray pointing at the bottom (Both rays are packed into the same "diagonal" ray value, but the lengths are calculated separately).
  // This gets us the uv extents for the rays that we cast, which we can use to scale our output UVs.
  float2 maxUV;
  {
