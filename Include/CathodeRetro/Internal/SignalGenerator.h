    ...
    void GeneratePhasesTexture()
    {
      ...
      generateSignalConstantBuffer->Update(
        GeneratePhaseTextureConstantData{
          .initialFrameStartPhase = float(frameStartPhaseNumerator) / float(sourceSettings.denominator),
          .prevFrameStartPhase = float(prevFrameStartPhaseNumerator) / float(sourceSettings.denominator),
          .phaseIncrementPerScanline = float(sourceSettings.phaseIncrementPerLine) / float(sourceSettings.denominator),
          .samplesPerColorburstCycle = k_signalSamplesPerColorCycle,
          .instabilityScale = artifactSettings.instabilityScale,
          .noiseSeed = noiseSeed,
          .signalTextureWidth = signalTexture->Width(),
          .scanlineCount = signalTexture->Height(),
        });
    ...
    void GenerateCleanSignal(const ITexture *rgbTexture)
    {
      ...
      generateSignalConstantBuffer->Update(
        RGBToSVideoConstantData{
          ...
          .inputWidth = rgbTexture->Width(),
          .outputWidth = signalTexture->Width(),
          .scanlineCount = signalTexture->Height(),
          .compositeBlend = (signalProps.type == SignalType::Composite) ? 1.0f : 0.0f,
          .instabilityScale = artifactSettings.instabilityScale,
          .noiseSeed = noiseSeed,
        });
    ...
    void ApplyArtifacts()
    {
      ...
      applyArtifactsConstantBuffer->Update(
        ApplyArtifactsConstantData {
          .ghostVisibility = artifactSettings.ghostVisibility,
          .ghostDistance = artifactSettings.ghostDistance,
          .ghostSpreadScale = artifactSettings.ghostSpreadScale,

          .noiseStrength = artifactSettings.noiseStrength,
          .noiseSeed = noiseSeed,

          .signalTextureWidth = scratchSignalTexture->Width(),
          .scanlineCount = scratchSignalTexture->Height(),
          .samplesPerColorburstCycle = k_signalSamplesPerColorCycle,
        });
