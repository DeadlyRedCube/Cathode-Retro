...
#include "NTSCRGBSignalGenerator.h"
#include "NTSCArtifacting.h"
#include "NTSCLumaChromaToRGB.h"
...
struct LoadedTexture 
{
  ...
  std::unique_ptr<NTSCRenderBuffers> ntscRenderBuffers;
  std::unique_ptr<NTSCRGBSignalGenerator> ntscSignalGenerator;
  std::unique_ptr<NTSCArtifacting> ntscArtifacting;
  std::unique_ptr<NTSCLumaChromaToRGB> ntscLumaChromaToRGB;
  std::unique_ptr<CRTEmulation> crtEmulation;
};

std::unique_ptr<LoadedTexture> loadedTexture;

NTSCPhaseGenerationInfo generationInfo = k_NESLikePhaseGenerationInfo;
...
void LoadTexture(wchar_t *path)
{
  ...
  load->ntscRenderBuffers = std::make_unique<NTSCRenderBuffers>(s_graphicsDevice.get(), width, height, generationInfo.colorCyclesPerInputPixel, generationInfo.denominator);
  load->ntscSignalGenerator = std::make_unique<NTSCRGBSignalGenerator>(
    s_graphicsDevice.get(),
    width,
    load->ntscRenderBuffers->signalTextureWidth,
    height);
  load->ntscArtifacting = std::make_unique<NTSCArtifacting>(s_graphicsDevice.get(), load->ntscRenderBuffers->signalTextureWidth, height);
  load->ntscLumaChromaToRGB = std::make_unique<NTSCLumaChromaToRGB>(s_graphicsDevice.get(), load->ntscRenderBuffers->signalTextureWidth, height);
  load->crtEmulation = std::make_unique<CRTEmulation>(s_graphicsDevice.get(), width, load->ntscRenderBuffers->signalTextureWidth, height);
...
int PASCAL WinMain( HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
  ...
  try
  {
    ...
    while (!done)
    {
      ...
      {
        loadedTexture->ntscSignalGenerator->Generate(
          s_graphicsDevice.get(),
          loadedTexture->srv,
          loadedTexture->ntscRenderBuffers.get(),
...
#if 1
        {
          ArtifactingOptions options;
          options.noiseScale = 0.15f;
          options.ghostVisibility = 0.65f;
          loadedTexture->ntscArtifacting->Apply(s_graphicsDevice.get(), loadedTexture->ntscRenderBuffers.get(), options);
        }
#endif

        {
          ScreenInfo info;
          info.blackLevel = 0.0f;
          info.whiteLevel = 1.0f;
          ...
          loadedTexture->ntscLumaChromaToRGB->Apply(s_graphicsDevice.get(), loadedTexture->ntscRenderBuffers.get(), info);
        }
...
#if 1
        {
          ...
          loadedTexture->crtEmulation->Render(s_graphicsDevice.get(), loadedTexture->ntscRenderBuffers.get(), params);
        }
#endif
      }
